package com.example.likeyoutube2.from_libs.utils;



import com.example.likeyoutube2.from_libs.DownloadProgressCallback;
    ...
    private void processOutputLine(String line) {
        ...
        if (m.matches()) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                progress = Float.parseFloat(m.group(GROUP_PERCENT));
            }
            long eta = 0;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                eta = convertToSeconds(m.group(GROUP_MINUTES), m.group(GROUP_SECONDS));
            }
